...
def enviar_alerta(nick, clan, score, lugar, slot_id, server_key):
    ...
    embed = {"title": "Slither ALERTA - Ranker acima de 50.000!", "color": 0x00FF88, "fields": [{"name": "Jogador", "value": f"**{nick}**{clan_texto}", "inline": True}, {"name": "Score", "value": f"**{formatar_score(score)} pontos**", "inline": True}, {"name": "Posicao", "value": f"#{lugar}", "inline": True}, {"name": "Servidor", "value": f"Slot {slot_id} - {server_key}", "inline": False}], "footer": {"text": "Slither Monitor - slithermania.com.br"}, "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
